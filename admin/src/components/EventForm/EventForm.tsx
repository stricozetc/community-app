...
export class EventForm extends React.Component<EventFormProps, EventFormState> {
  ...
  public isValidate(): boolean {
    ...
    if (!this.state.location) {
      locationErrors.push(frontEndValidationEventRegister.location.length);
    } else {
      locationErrors = this.removeElFromArrByValue(
        locationErrors,
        frontEndValidationEventRegister.location.length
      );
    }

    if (this.state.location.length < MIN_LENGTH_LOCATION || this.state.location.length > MAX_LENGTH_LOCATION) {
      locationErrors.push(frontEndValidationEventRegister.location.length);
    } else {
      locationErrors = this.removeElFromArrByValue(
        locationErrors,
